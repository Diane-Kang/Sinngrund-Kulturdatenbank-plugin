...
class SinngrundKulturdatenbank
{
  ...
  function fill_custom_meta_box_content($post)
  {
    ...
    if ($currentTaxonomyValues) {
      foreach ($currentTaxonomyValues as $value)
        array_push($currentTaxonomytermids, $value->term_id);
    }
    ...
    <p>
      <?php foreach ($terms as $term): ?>
        <input type="checkbox" name="orte[]" value="<?php echo $term->name; ?>"
          <?php if (in_array($term->term_id, $currentTaxonomytermids))
            echo "checked"; ?>>
        <?php echo $term->name; ?>
        </input><br />
      <?php endforeach; ?>
    ...
    switch ($name) {
      ...
      case 'valid':
        ...
        if ($this->post_valid_check($category_name, $lati, $longi)) {
          ...
          echo "X: Error mit Geocode oder/and Kategory";
        break;
      case 'route':
